...
const ActionScheduleScreen = ({ route }) => {
	...
	const { loaderOn, loaderOff, showDialogConfirmation, showDialogMessage } =
		useModal();
		...
		switch (route.params.action) {
			case 'OPEN':
				showDialogConfirmation(
					'door-open',
					'Open Class',
					'Are you sure to open this class now?',
					() => {
						openClass();
					},
					() => {
			...
			case 'CLOCK_IN':
				showDialogConfirmation(
					'door-open',
					'Clock In',
					'Are you sure to clock in this class now?',
					() => {
						clockInClass();
					},
					() => {
			...
			case 'CLOSE':
				showDialogConfirmation(
					'door-closed',
					'Close Class',
					'Are you sure to close this class now?',
					() => {
						closeClass();
					},
					() => {
			...
			case 'CLOCK_OUT':
				showDialogConfirmation(
					'door-open',
					'Open Class',
					'Are you sure to clock out this class now?',
					() => {
						clockOutClass();
					},
					() => {
	...
	const loadScheduleDetail = async () => {
		...
		await axiosPrivate
			.get(`/schedule/${route.params.scheduleId}`)
			.then((response) => {
	...
	const openClass = useCallback(async () => {
		...
		if (facesData.length > 1) {
			showDialogMessage(
				'error',
				'FEE0003',
				'Terdeteksi beberapa wajah. Pastikan hanya satu wajah yang terlihat di dalam bingkai kamera!'
			);
		} else if (facesData.length < 1) {
			showDialogMessage(
				'error',
				'FEE0002',
				'Tidak ada wajah yang terdeteksi. Pastikan wajah Anda terlihat jelas di dalam bingkai kamera!'
			);
		} else {
			console.log(`openClass`);
			loaderOn();
			await axiosPrivate
				.patch('/schedule/activate', {
	...
	useEffect(() => {
		(async () => {
			...
			if (!permission.granted) {
				console.log(
					`[NOT GRANTED] cameraPermission: ${JSON.stringify(
						permission
					)}`
				);
				showDialogMessage(
					'error',
					'FEE0003',
					'Samper App has no permission to access device camera!',
					() => {
						navigation.dispatch(
							CommonActions.reset({
								index: 0,
	...
	return (
		<>
			<View style={styles.container}>
				...
				>
					<View
						style={[styles.clock, { backgroundColor: '#D8261D' }]}
					>
						<View
							style={{
						...
						>
							...
							>
								...
								>
									...
									route.params.action === 'CLOCK_IN'
										? currentTime
										: route.params.clockIn
										? moment(route.params.clockIn).format(
												'HH:mm'
                                        )
										: route.params.openTime
										? moment(route.params.openTime).format(
												'HH:mm'
                                        )
										: '--:--'}
								</Text>
							...
							>
								...
								>
									{route.params.action === 'CLOSE' ||
									route.params.action === 'CLOCK_OUT'
										? currentTime
										: route.params.clockOut
										? moment(route.params.clockOut).format(
												'HH:mm'
										)
										: route.params.closeTime
										? moment(route.params.closeTime).format(
												'HH:mm'
                                        )
										: '--:--'}
								</Text>
					...
					<View style={styles.overlay}>
						...
						<Text style={styles.instruction}>
							Luruskan wajah dengan bingkai kamera
						</Text>
					</View>
				</CameraView>
				<View
